...
{
    [Header("얼음")]
    [SerializeField] private GameObject _everFrostPrefab;
    ...
    {
        GameObject frostObj = Instantiate(_everFrostPrefab, new Vector3(pos.x, pos.y, 0), Quaternion.identity);

        Frost frostComp = frostObj.GetComponent<Frost>();
        if (frostComp != null)
        {
            frostComp.Initialize(ComponentContainer.Get<EntityStat>(), _damageStat);
            frostComp.EverFrost();
        }
